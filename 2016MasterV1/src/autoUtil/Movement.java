...
public class Movement {
	...
	public static void orient(double angle, double yaw, AHRS navX) {
		...
		/**
		 * Booleans if yaw is greater than or less than the needed angle in a +- 5 range
		 */
	...
	public static void driveDistance(double dist) {
		...
		if (resetEncoders) { //Tests if encoders needs to be reset. Resets encoder if applicable
	 ...
	 * 
	 * @Postcondition the robot will spin in place with the designated side's motors moving forward and
	 * the other side's motors moving backwards.
	 */
	public static void turnInPlace(String dir, double speed) {
		if (dir.equals(LEFT)) { //Tests if robot needs to turn left
			Robot.subState = "Turning Left...";
			
			Motors.motorDriveFrontLeft.set(speed); Motors.motorDriveBackLeft.set(speed);
			Motors.motorDriveFrontRight.set(-speed); Motors.motorDriveBackRight.set(-speed);
		}
		
		else if (dir.equals(RIGHT)) { //Tests if robot needs to turn right
